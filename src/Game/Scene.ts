...
export class Scene {
    ...
    event: { event: GameEvent } = { event: null as unknown as GameEvent }
    ...
    constructor() {
        ...
        // this.player = new MapWriter()
        this.map = new MapData(mapdata)

        this.event.event = new EventFirst(this.map)
    ...
    modePlay() {
        this.map.realSprites.forEach((s) => s.update(this.map, this.map.player))
    ...
    #action() {
        ...
        const s = this.map.realSprites.find((s) => {
            for (let i = 0; i < s.size.y; i++) {
                for (let j = 0; j < s.size.x; j++) {
                    if (s.p.add(vec(j, i)).equals(this.map.player.p.add(d[this.map.player.direction]))) return true
                }
            }
        })
    ...
    modeMenu() {
        ...
        if (keyboard.longPressed.has("ArrowUp")) {
            ...
            document.querySelectorAll(`#menu #left #buttons button`).forEach((b) => b.classList.remove("selected"))
            document.querySelector(`#menu #left #buttons :nth-child(${this.menu.index + 1})`)?.classList.add("selected")
        }
